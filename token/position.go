...
var (
	...
	ErrIllegalBaseOrSize   = errors.New("illegal base or size")
	ErrTokenOffsetOverflow = errors.New("token.Pos offset overflow (> 2G of source code in file set)")
)
...
// p must be a Pos value in that file or NoPos.
func (f *File) Position(p Pos) (pos Position, err error) {
	if p != NoPos {
		if int(p) < f.base || int(p) > f.base+f.size {
			return pos, ErrIllegalPosValue
		}
		pos = f.position(p)
	}
	return pos, nil
...
func (s *FileSet) AddFile(filename string, base, size int) (*File, error) {
	...
	if base < s.base || size < 0 {
		return nil, ErrIllegalBaseOrSize
	}
	// base >= s.base && size >= 0
	f := &File{s, filename, base, size, []int{0}, nil}
	base += size + 1 // +1 because EOF also has a position
	if base < 0 {
		return nil, ErrTokenOffsetOverflow
	}
